# -*- coding: utf-8 -*-
"""DEFINITIVE CODE.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/14QFYGRn3UZDThm0aiLrQ5rplJ3MPTg4D
...
def plots_stats(data, var):
    ...
    # --- Figure 3: Monthly trends across years ---
    fig, axs = plt.subplots(2, 1, figsize=(16,10))
...
def OU_with_jumps(series, trend, dt=1.0, pl1=0, pl2=0, plot_simulation=1):
    ...
    # --- 2️⃣ Prepare trend without jumps ---
    trend_clean = approx.drop(index=approx.index[jumps])

    # --- 3️⃣ Interpolate trend to restore continuity ---
    ...
    # --- 4️⃣ OU ESTIMATION ON RESIDUALS (clean only) ---
    ...
    # --- 5️⃣ Jump increments estimation ---
...
def OU_Hawkes(series, trend=None, dt=1.0, hawkes_graph=True, plots=True, scatter=False, seed=None):
    ...
    # --- 3️⃣ OU estimation on clean series ---
    ...
    # --- 7️⃣ Reconstruct OU + Hawkes series ---
    x_rec = np.zeros(N)
    x_rec[0] = s.iloc[0]+ trend[0]
...
def MonteCarlo_option_payoffs(models, time, main, test, steps=366, n_sim=10000, plot=True, option_type='call'):
    ...
    # === 2. Compute progressive mean of simulated payoffs === #
    ...
    # === 3. Define indices and compute strikes === #
    indices = ["HDD", "CDD", "CAT"]
    ...
    # === 4. Compute simulated and actual payoffs === #
    simulated = {
        model_name: {
    ...
    # === 5. Compute errors for convergence === #
    convergence_errors = {}
    for opt in options:
    ...
    # === 6. Plotting (facoltativo, senza errore) === #
    if plot:
...
def mc_metrics_from_simulations(sims):
    """
    sims: np.ndarray shape (num_sim, sim_length)
    Calcola MCSE, varianza, delta ecc.
    """
    num_sim = sims.shape[0]

    # Media delle simulazioni (una media per simulazione)
    sim_means = sims.mean(axis=1)

    # -------------------------------
    # MCSE e varianza del campionamento
    # -------------------------------
    sampling_variance = np.var(sim_means, ddof=1)
    mcse = np.sqrt(sampling_variance / num_sim)

    # -------------------------------
    # Delta tra N e 2N simulazioni
    # -------------------------------
    N = num_sim // 2  # metà simulazioni
    if N > 0:
        mean_N = np.mean(sim_means[:N])
        mean_2N = np.mean(sim_means[:2*N])
        delta = abs(mean_N - mean_2N) / abs(mean_2N)
    else:
        delta = np.nan

    results = {
        "MCSE": mcse,
        "SamplingVariance": sampling_variance,
        "Delta_N_vs_2N": delta,
        "MeanOverall": np.mean(sim_means),
        "MeansPerSimulation": sim_means,
    }
    return results
...
# 1. Crea una cartella in /content
os.makedirs("/content/images", exist_ok=True)

# 2. Sposta tutte le immagini che iniziano con Oslo in /content/images
for filename in os.listdir("/content"):
    if filename.startswith("Bahrain") and filename.endswith((".png", ".jpg", ".jpeg")):
        shutil.move(f"/content/{filename}", f"/content/images/{filename}")

# 3. Crea il file ZIP in /content
shutil.make_archive("/content/bahrain_figures", "zip", "/content/images")

# 4. Scarica lo ZIP
files.download("/content/bahrain_figures.zip")

evaluate_simulation_models(sim_mode,testing)

def mc_metrics_from_dict(sims_dict):
    results = {}
    for model, sims in sims_dict.items():
        num_sim = sims.shape[0]
...
# Esempio di uso
# -------------------------------
# sims_dict è il tuo dizionario
metrics_results = mc_metrics_from_dict(sim_mode[1])
